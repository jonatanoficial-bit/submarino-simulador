...
function renderStaticTexts(){
  const map = {
    ...
    captainLogTitle:'captainLog', saveBtn:'save', resetBtn:'reset', fireBtn:'fire', toggleDepthBtn:'depth', endMissionBtn:'abort'
  };
...
function renderLobby(){
  ...
  const summary = [`Version: ${state.version}`,`Build: ${state.buildTime}`,`Phase: ${state.phase}`,`Owned submarines: ${state.submarines.filter(s => s.owned).length}/${state.submarines.length}`,`Selected mission difficulty: ${getMissionById(state.selectedMissionId).difficulty}/3`];
...
function renderIntel(){
  setHTML('intelContent', ['Escort vessels push aggressively once inside engagement range.','Enemy submarines are harder to detect and attack more selectively.','Civilian targets fail the mission if destroyed.','Advanced submarines improve stealth and torpedo capacity.'].join('\n\n'));
  setHTML('promptPackContent', ['[UI] WWII naval tactical HUD, brushed metal, teal sonar glow, premium strategy game interface, multilingual, clean commercial style','[VESSEL] WWII escort destroyer, realistic naval rendering, top-down strategy asset, crisp silhouette, ocean background','[VESSEL] WWII cargo ship, realistic top-down game asset, clean shadow separation, neutral ocean background','[PORTRAIT] submarine mechanic portrait, cinematic military lighting, realistic, game asset, no background logos'].join('\n\n'));
}

function renderAssets(){
  setHTML('assetsContent', ['Suggested additions for next art pass:','assets/images/vessels','assets/images/ui','assets/images/portraits','assets/audio/sonar','assets/audio/explosions','','Use stable filenames to swap generated art without code changes.'].join('\n'));
}
...
export function renderAll(){
  renderLanguageSelect(); renderStaticTexts(); renderSidebar(); renderLobby(); renderMissions(); renderCareer(); renderIntel(); renderAssets(); renderGamePanels();
