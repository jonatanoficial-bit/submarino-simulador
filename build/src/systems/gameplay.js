...
function createEnemies(mission){
  ...
  sequence.forEach((type, i) => {
    ...
    list.push({
      id:`enemy_${i}`, ...tpl,
      x:560 + i * 70, y:100 + (i * 88) % 340,
      hull: tpl.hull + mission.difficulty * 8,
      attackCooldown: 140 + Math.random() * 120,
      waypoint: { x: 620 + Math.random()*220, y: 80 + Math.random()*380 },
      detected:false
    });
  });
  return list;
...
function enemyActions(){
  ...
  for (const e of state.game.enemies){
    ...
    if (e.type === 'cargo' || e.type === 'civilian'){
      ...
      if (Math.hypot(e.x - e.waypoint.x, e.y - e.waypoint.y) < 20){
        e.waypoint = { x: 560 + Math.random()*300, y: 60 + Math.random()*420 };
      }
    }
    if (e.type === 'escort'){
...
export function fireTorpedo(){
  ...
  const p = state.game.player;
  if (p.torpedoes <= 0){ state.logs.unshift('[WEAPON] No torpedoes remaining.'); return; }
  if (p.tubes <= 15){ state.logs.unshift('[WEAPON] Torpedo tubes damaged.'); return; }
  p.torpedoes--;
  ...
  const chance = Math.max(0.15, Math.min(0.92, 0.84 - distance/900 + p.sonar/250 - (100-p.tubes)/190 + (p.submerged?0.06:0)));
